...
export class DeclarationsService extends BaseService {
    ...
    getDeclarations(filter: string = null): Observable<Array<DeclarationHeader>> {
        ...
        return this.http.get<Array<DeclarationHeader>>("declaration" + prefix + filter).pipe(catchError(this.handleError),finalize(() => this.triggerLoading(false)));
    }

    getDeclaration(id:string): Observable<DeclarationViewModel>{
        this.triggerLoading(true);
        return this.http.get<DeclarationViewModel>(`declaration/${id}`).pipe(catchError(this.handleError),finalize(() => this.triggerLoading(false)));
    }
}